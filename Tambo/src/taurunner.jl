...
earth = tr.body.earth.construct_earth(; layers=[(4.0, 1.0)])
...
output = tr.main.run_MC(energies, thetas, earth, xs, prim_prop, rand)
...
atmosphere = tr.body.Body(0.001225, 100000.0)   #body(density [g/cm^3], radius [km])
...
track = tr.track.Chord(; theta=0.0, depth=0.0)

tau_out = []
for tau in exiting_taus
    this_tau = tr.particle.Particle(
        tau["PDG_Encoding"],
        tau["Eout"],
        0.0,
        0.0,
