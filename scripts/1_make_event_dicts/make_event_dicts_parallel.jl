...
function make_hit_map(
    ...
    println("making hit map for event $event_number")
    ...
    for file in files
        ...
        println("xmin: $(xmin/units.km), xmax: $(xmax/units.km), ymin: $(ymin/units.km), ymax: $(ymax/units.km)")
        println("event x mean: $(mean(df.x)/units.km)")
        println("event y mean: $(mean(df.y)/units.km)")
        df = filter(
...
function main()
    ...
    config = SimulationConfig(; Dict(k=>v for (k, v) in sim["config"])...)
    ...
    for (_, event_number) in enumerate(event_numbers)
        println("This is event number: $event_number")
        if can_skip_event(event_number, outfile, run_desc)
            println("skipped event $event_number")
            continue
        end
        println("event number $event_number is not skipped")
